...
# set user action will give the option to distiguish the user who usinf the apps, if user is seller or buyer or returning user
  def set_user
  ...
  def after_sign_in_path_for(profile)
    if (current_user.profile)
      if (params[:user][:user_type] == "returning_user")
        root_path
